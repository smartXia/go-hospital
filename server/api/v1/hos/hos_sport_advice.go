...
// GetCurrentHosSportAdviceList 分页获取hosSportAdvice表列表
// @Tags HosSportAdvice
// @Summary 分页获取hosSportAdvice表列表
// @Security ApiKeyAuth
// @accept application/json
...
// @Success 200 {string} string "{"success":true,"data":{},"msg":"获取成功"}"
// @Router /hosSportAdvice/getHosSportAdviceList [get]
func (hosSportAdviceApi *HosSportAdviceApi) GetCurrentHosSportAdviceList(c *gin.Context) {
	var pageInfo hosReq.HosSportAdviceSearch
	err := c.ShouldBindQuery(&pageInfo)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	if list, total, err := hosSportAdviceService.GetCurrentHosSportAdviceList(pageInfo, c); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
		response.OkWithDetailed(response.PageResult{
