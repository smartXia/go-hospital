...
type HosSportAdvice struct {
	global.GVA_MODEL
	FlowId    *int   `json:"flowId" form:"flowId" gorm:"column:flow_id;comment:流程id;size:10;" binding:"required"`   //流程id
	UserId    *int   `json:"userId" form:"userId" gorm:"column:user_id;comment:患者id;size:10;" binding:"required"`   //患者id
	Name      string `json:"name" form:"name" gorm:"column:name;comment:名称;size:500;"`                              //名称
	Period    string `json:"period" form:"period" gorm:"column:period;comment:周期;size:500;" binding:"required"`     //周期
	...
	SyncWx    *int   `json:"syncWx" form:"syncWx" gorm:"column:sync_wx;comment:是否同步微信;size:10;" binding:"required"` //是否同步微信
