<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="部位" prop="buwei">
         <el-input v-model="searchInfo.buwei" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="体段" prop="tiduan">
         <el-input v-model="searchInfo.tiduan" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="行动" prop="xingdong">
         <el-input v-model="searchInfo.xingdong" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="方向" prop="fangxiang">
         <el-input v-model="searchInfo.fangxiang" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="位置" prop="weizhi">
         <el-input v-model="searchInfo.weizhi" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="部位" prop="buwei" width="120" />
        <el-table-column align="left" label="体段" prop="tiduan" width="120" />
        ...
        <el-table-column align="left" label="关联图片" prop="relationPhotos" width="120" />
        <el-table-column align="left" label="relationVideos字段" prop="relationVideos" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHosSportModeFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="relationVideos字段:"  prop="relationVideos" >
              <el-input v-model="formData.relationVideos" :clearable="true"  placeholder="请输入relationVideos字段" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getHosSportModeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteHosSportModeByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHosSportModeFunc = async(row) => {
    const res = await findHosSportMode({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHosSportModeFunc = async (row) => {
    const res = await deleteHosSportMode({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
