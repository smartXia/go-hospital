<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="请求状态" prop="status" width="120" />
        <el-table-column align="left" label="用户id" prop="userId" width="120" />
        <el-table-column align="left" label="操作" prop="action" width="120" />
        <el-table-column align="left" label="系统" prop="os" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateSysOperationRecordsFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="请求状态:"  prop="status" >
              <el-input v-model.number="formData.status" :clearable="true" placeholder="请输入请求状态" />
            </el-form-item>
            <el-form-item label="延迟:"  prop="latency" >
              <el-input v-model.number="formData.latency" :clearable="true" placeholder="请输入延迟" />
            </el-form-item>
...
import {
  ...
  deleteSysOperationRecords,
  deleteSysOperationRecordsByIds,
  updateSysOperationRecords,
  findSysOperationRecords,
  getSysOperationRecordsList
...
const getTableData = async() => {
  const table = await getSysOperationRecordsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSysOperationRecordsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysOperationRecordsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysOperationRecordsFunc = async(row) => {
    const res = await findSysOperationRecords({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysOperationRecordsFunc = async (row) => {
    const res = await deleteSysOperationRecords({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
