<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="出生状态" prop="birthStatus" width="120" />
        <el-table-column align="left" label="出生方式" prop="birthMode" width="120" />
        <el-table-column align="left" label="发育状态" prop="growStatus" width="120" />
        <el-table-column align="left" label="家族病史" prop="familyMedical" width="120" />
        <el-table-column align="left" label="历史病史" prop="medicalHistory" width="120" />
        <el-table-column align="left" label="病灶记录" prop="lesionHistory" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHosScaleFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getHosScaleList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteHosScaleByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHosScaleFunc = async(row) => {
    const res = await findHosScale({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHosScaleFunc = async (row) => {
    const res = await deleteHosScale({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
