<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="量表录入id" prop="scaleId">
             ...
             <el-input v-model.number="searchInfo.scaleId" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="现场关联id" prop="askId">
            
             <el-input v-model.number="searchInfo.askId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="所属状态" prop="step" width="120" />
        <el-table-column align="left" label="量表录入id" prop="scaleId" width="120" />
        <el-table-column align="left" label="现场关联id" prop="askId" width="120" />
        <el-table-column align="left" label="运动建议id" prop="adviceId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHosFlowFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="量表录入id:"  prop="scaleId" >
              <el-input v-model.number="formData.scaleId" :clearable="true" placeholder="请输入量表录入id" />
            </el-form-item>
            <el-form-item label="现场关联id:"  prop="askId" >
              <el-input v-model.number="formData.askId" :clearable="true" placeholder="请输入现场关联id" />
            </el-form-item>
            <el-form-item label="运动建议id:"  prop="adviceId" >
              <el-input v-model.number="formData.adviceId" :clearable="true" placeholder="请输入运动建议id" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getHosFlowList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteHosFlowByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHosFlowFunc = async(row) => {
    const res = await findHosFlow({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHosFlowFunc = async (row) => {
    const res = await deleteHosFlow({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
