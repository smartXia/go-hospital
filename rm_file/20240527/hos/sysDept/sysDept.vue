<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="父级部门" prop="parent" width="120" />
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="父级部门:"  prop="parent" >
              <el-input v-model.number="formData.parent" :clearable="true" placeholder="请输入父级部门" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getSysDeptList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysDeptByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysDeptFunc = async(row) => {
    const res = await findSysDept({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysDeptFunc = async (row) => {
    const res = await deleteSysDept({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
