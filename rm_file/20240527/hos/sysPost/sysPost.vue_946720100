<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="岗位标识" prop="code" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateSysPostFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="岗位标识:"  prop="code" >
              <el-input v-model="formData.code" :clearable="true"  placeholder="请输入岗位标识" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getSysPostList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysPostByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysPostFunc = async(row) => {
    const res = await findSysPost({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.resysPost
...
// 删除行
const deleteSysPostFunc = async (row) => {
    const res = await deleteSysPost({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createSysPost(formData.value)
                  break
                case 'update':
                  res = await updateSysPost(formData.value)
                  break
                default:
                  res = await createSysPost(formData.value)
                  break
