    ...
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="打卡id" prop="clockId">
             ...
             <el-input v-model.number="searchInfo.clockId" placeholder="搜索条件" />
...
import {
  ...
  deleteHosSportClockCommit,
  deleteHosSportClockCommitByIds,
  updateHosSportClockCommit,
  findHosSportClockCommit,
  getHosSportClockCommitList
...
const getTableData = async() => {
  const table = await getHosSportClockCommitList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHosSportClockCommitFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteHosSportClockCommitByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHosSportClockCommitFunc = async(row) => {
    const res = await findHosSportClockCommit({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHosSportClockCommitFunc = async (row) => {
    const res = await deleteHosSportClockCommit({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
